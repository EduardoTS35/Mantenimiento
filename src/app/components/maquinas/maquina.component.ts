...
import { dialogMaquinas } from 'src/app/common/dialogMaquinas/dialogMaquinas.component';
...
export class MaquinaComponent implements OnInit {
    ...
    constructor(public dialog:MatDialog,
                ...
                private maquinariaService: ApiMaquinariaService, 
    ...
    getMaquinaria(){
        this.maquinariaService.getMaquinaria().subscribe( response=>{
    ...
    openAdd(){
    const dialogRef=this.dialog.open(dialogMaquinas,{
    });
  ...
  openEdit(maquina:Maquinas){
    this.maquinariaService.getMaquina(maquina);
    const dialogRef=this.dialog.open(dialogMaquinas,{      
    });
    ...
    deleteMaquina(maquina: Maquinas) {
        ...
        const dialogRef=this.dialog.open(DialogDeleteComponent,{
          ...
          dialogRef.afterClosed().subscribe(result=>{
            if(result===true){
              this.maquinariaService.deleteMaquinaria(maquina.idMaquina).subscribe(response=>{
                if(response.exito === 1){
